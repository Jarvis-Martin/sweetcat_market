package com.sweetcat.takewayorder.application.event.subscribe;
...
@RocketMQMessageListener(
        ...
        consumerGroup = "CG_SWEETCAT_TAKEAWAY_ORDER_USER_CHANGED_ADDRESS",
...
public class UserChangedAddressEventSubscriber implements RocketMQListener<UserChangedAddressEvent> {
    ...
    public void onMessage(UserChangedAddressEvent event) {
        System.out.println("sweetcat-takeaway-order 接收领域事件 UserChangedAddressEvent 时间为：" + Instant.now().toEpochMilli());
        ...
        orderApplicationService.changeCustomerAddress(command);
    }
